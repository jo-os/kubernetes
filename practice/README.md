...
```
  trafficPolicy:
    tls:
      mode: ISTIO_MUTUAL
```
**Kiali** - компонент который в графической форме показывает межсервисное взамодействие. Представляет собой web приложение который устанавливает в кластер вместе с istio

**Возможности**
- схема тополоогии сервисов
- сбор метрик (время ответа, количество ошибок )
- проверка конфигурации объктов

Kiali состоит из Front-end и Back-end. Back-end взаимодействует с kubernetes api server и prometheus
```
kubectl apply -f samples/addons/kiali.yaml
kubectl apply -f samples/addons/prometheus.yaml
sudo kubectl --namespace istio-system port-forward kiali-545878ddbb-75w4l 20001 --address='0.0.0.0'
= istioctl dashboard kiali # откроет на хосте браузер c kiali
while true; do curl -s http://192.168.0.50/productpage > /dev/null;sleep 1;done
в Graph - Display - включаем Response Time, Traffic Animation
```
**Трассировка запросов**
- найти узкие места в производительности
- наглядно увидеть последовательность вызовов

Как это работает - каждый запрос получает специальный идентификатор, который обычно сохраняется в заголовках запросов. Каждый запрос представляет собой подзапрос - которые факстически и представляют работу. Такие подзапросы называются **Span** - это единица операции. У Spana есть характеристики - имя, время начала, продолжительность, уникльный id, метаданные (параметры вызова или текст ошибки), id родительского Span. **Trace** - это коллекция Span'ов (например пользовательский запрос состоит из подзапросов - api, service, datenbank - они все входят в 1 трейс)

**Системы трассировки:**
- Zipkin
- Jaeger
- Облачные системы - AWS X-Ray, Azure Monitor, Cloud Trace

**OpenTelemetry** - независимый стандарт Tracing'а - описывает как и что должно собираться, не привязываясь к конкретной реализации в каком либо языке.

 **Jaeger:**
- jaeger agent - агент стоит на каждой машине и в него логируют сервисы на локальный дефолтный порт
- jaeger collector - на него все агенты посылают собранные трейсы а он кладет их в бд
- bd - cassandra, elasticsearch возможны  другие in memory бд
- jaeger UI - веб интерфейс для поиска и просмотра трейсов
```
kubectl apply -f samples/addons/jaeger.yaml
kubectl -n istio-system describe service tracing
